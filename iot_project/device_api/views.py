    ...
    def get(self, request, format=None):
        ...
        try:
            ...
            if device.is_registered:
                # If device is already registered, return 409 Conflict (previous behavior, kept)
            ...
            # --- MODIFIED LINE BELOW: Increased 'last_seen' threshold ---
            # Change 3 seconds to a more reasonable value, e.g., 30 seconds or 300 seconds (5 minutes)
            # This allows more time for the device's last_seen to be updated by the /data/ endpoint.
            if not device.is_online or (timezone.now() - device.last_seen).total_seconds() > 30: # Changed from 3 to 30
